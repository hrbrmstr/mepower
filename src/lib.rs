...
fn get(url: &str) -> Result<String, Error> {
	...
	Ok(client.get(url).header("User-Agent", UA).send()?.text()?)
}
...
pub async fn get_outages() -> Vec<OutageRecord> {
			...
			Ok(doc) => {
				...
				if counties.is_empty() {
					let no_data = OutageRecord {
						...
						message: String::from("No outage data found."),
					};

					outages.push(no_data);
				} else {
